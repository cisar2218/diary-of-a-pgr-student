...
void drawWindowContents() {
  ...
  if(gameState.freeCameraMode == true) {
    ...
    //// a) using spherical coordinates - from scratch
    //float theta = glm::radians(gameState.cameraElevationAngle);
    //float phi = glm::radians(gameObjects.spaceShip->viewAngle);

    //cameraViewDirection = glm::vec3(  // must use =
    //  cos(theta) * cos(phi),
    //  cos(theta) * sin(phi),
    //  sin(theta)
    //);
    //cameraUpVector = glm::vec3(
    //  -sin(theta) * cos(phi),
    //  -sin(theta) * sin(phi),
    //  cos(theta)
    //);

    // a) using spherical coordinates - just rotate up (rotation by phi is done in turnSpaceShipRight())
    //cameraViewDirection.x *= cos(theta);   // use *=, as direction has already been rotated by phi
    //cameraViewDirection.y *= cos(theta);
    //cameraViewDirection.z = sin(theta);   // use =, as z was set to zero and *= would not change this zero

    //cameraUpVector.x = -sin(theta) * cos(phi);  // same = as above!!!
    //cameraUpVector.y = -sin(theta) * sin(phi);
    //cameraUpVector.z = cos(theta);
