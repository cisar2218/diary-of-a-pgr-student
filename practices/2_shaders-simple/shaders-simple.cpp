...
void onDisplay() {
  ...
  if( true )
    projection = glm::rotate(glm::mat4(1.0f), glm::radians(state.time * 60.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // the animation
  else
  ...
  glUniform1i(locations.winWidth, WIN_WIDTH);
...
bool loadShaders() {
  ...
  // use the glGetUniformLocation function to obtain locations to winWidth and alpha uniforms
...
int main(int argc, char* argv[]) {
  ...
  // if(!pgr::initialize(pgr::OGL_VER_MAJOR, pgr::OGL_VER_MINOR, pgr::DEBUG_OFF))  //varianta pro nSight debugger
