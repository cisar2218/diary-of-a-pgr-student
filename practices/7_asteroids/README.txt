...
* change position and direction of the reflector (spotlight) to be the same as
  ...
  - send correct position and direction of the reflector to the shaders
    (through mentioned uniforms)
	=> function drawWindowContents() in file asteroids.cpp
...
* normals have to be converted into the camera space and then passed from the
...
* equations are the same as in the previous task except for the evaluation on
  per-pixel basis (done through normal vector and position interpolation)
