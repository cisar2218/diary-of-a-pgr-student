...
* Texture animation is accomplished by changing texture coordinates over 
  ...
  Texture layout is stored in uniform named "pattern".
  >>> TASK 4_2-3 <<<
...
* Explosion texture has to be blended with the background. You have to enable 
  ...
  -> drawExplosion() in file render_stuff.cpp.
  >>> TASK 4_2-2 <<<
...
* At the beginning, the "game over" text has to appear on the left and it 
  should move to the right. When it completely rolls out the window, it has
  to emerge from the window left side again.
  >>> TASK 4_3-4 <<<
...
* Triangle strip has to be subdivided in the direction of the u texture
  ...
  bannerVertexData[] in file data.h.
  >>> TASK 4_3-1 <<<
...
TASK 4_2:
 -> data.h: 260
 -> render_stuff.cpp: 279
 -> explosion.frag: 20
TASK 4_3:
 -> data.h: 279
 -> render_stuff.cpp: 351, 783
 -> banner.vert: 21
